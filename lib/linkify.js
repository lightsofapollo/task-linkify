...
var debug = require('debug')('linkify'),
    ...
    pullRequestAttached = require('./pull_request_attached'),
...
function createRedirect(bz, github, user, repo, prNum, bugNum, callback) {
  ...
  github.pullRequests.get(msg, function(err, data) {
    ...
    var contents = new Buffer(data.html_url).toString('base64');
    ...
    var attachment = {
      ...
      summary: data.base.label + ' PR#' + data.number,
      file_name: fileName,
...
function checkRedirect(bz, github, user, repo, prNum, bugNum, callback) {
  bz.bugAttachments(bugNum, function(err, list) {
    if (err) return callback(err);

    // if there is already a github pr attachment skip
    if (pullRequestAttached(user, repo, prNum, list)) {
      debug('pull request already exists');
...
function link(bz, github, user, repo, prNum, callback) {
  ...
  github.pullRequests.getCommits(msg, function(err, data) {
    ...
    return checkRedirect(bz, github, user, repo,
